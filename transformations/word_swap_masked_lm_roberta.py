...
class WordSwapMaskedLM_roberta(WordSwap):
    ...
    Choose which method to use by specifying "bae" or "roberta-attack" for `method` argument.
    """

    def __init__(
        self,
        method="bae",
        state_dict = torch.load("./examples/language-modeling/output_finetunning_imdb_1000/pytorch_model.bin"),
        masked_language_model = RobertaForMaskedLM.from_pretrained(pretrained_model_name_or_path='roberta-base', state_dict=state_dict, cache_dir='./data'),
        max_length=256,
        max_candidates=50,
        **kwargs,
    ):
    ...
    def _get_replacement_words(self, current_text, index, **kwargs):
        ...
        elif self.method == "roberta-attack":
